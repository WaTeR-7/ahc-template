...
echo "next:"
echo "  1) 問題ページ(要ログイン+参加登録)を『完全な形で保存』→ $dest/problem/ に置く"
echo "     └ 機械取得できないのはこの1回だけ。以降は保存HTML内のCDN URLから自動。"
echo "  2) cd $dest && scripts/fetch_tools.sh   # tools.zip をDL→展開→build→in生成"
echo "  3) vis 出力に合わせ scripts/test.sh の SCORER を確認, LOG.md §1 を記入"
echo "  4) cp src/bin/00_base.rs src/bin/01_<approach>.rs で着手(§0チェックリスト)"
